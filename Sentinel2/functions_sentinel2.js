 ...
 *      MaskClouds
 * @description
 *      Cloud Mask with image of clouds
 */
exports.maskClouds = function(image) {
  var nube = ee.Image(image.get('cloud_mask')).select('probability');
  var no_nube = nube.lt(70);
  return image.updateMask(no_nube);
}
 ...
 * */
exports.maskEdges = function(s2_image) {
...
exports.computeNDWI = function (image) {
	...
	var exp = 'float(b("B8A") - b("B11"))/(b("B8A") + b("B11"))';
...
exports.computeEVI = function(image){
	...
	var exp = '2.5 * ((b("B8A") - b("B4")) / (b("B8A") + 6 * b("B4") - 7.5 * b("B2") + 1))';
